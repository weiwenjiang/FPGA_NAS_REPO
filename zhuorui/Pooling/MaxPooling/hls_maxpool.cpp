...
void maxpool_LOAD_IFM(hls::stream<DMA_DATA_128B_FIX> &input_dma_I,
		FPGA_DATA_FIX IFM[2][Tn_8][K_maxpool*Tr_maxpool][K_maxpool*Tc_maxpool],
		int custom_Tr,
		int custom_Tc,
		int custom_k,
		int custom_Tn
	...
	ifm1:for(int i=0;i<divided_Tn_8;i++){
		ifm2:for(int j=0;j<K_maxpool*custom_Tr;j++){
			ifm3:for(int m=0;m<K_maxpool*custom_Tc;m++){
...
void maxpool_FIRE(FPGA_DATA_FIX IFM[2][Tn_8][K_maxpool*Tr_maxpool][K_maxpool*Tc_maxpool],
			FPGA_DATA_FIX OFM[Tm][Tr_maxpool][Tc_maxpool],
			int row,
			int col,
			int custom_Tr,
			int custom_Tc,
...
#pragma HLS loop_tripcount min=1 max=Tc avg=Tc
			for(int i=0;i<K_maxpool;i++){
				for(int j=0;j<K_maxpool;j++){
#pragma HLS PIPELINE II=1
#pragma HLS dependence variable=OFM inter false
					for(int tii=0;tii<custom_Tn;tii++){
						if(tii<Tn/2){
							if(i==0 && j==0)
								OFM[tii][trr][tcc] = IFM[0][tii][K_maxpool*trr+i][K_maxpool*tcc+j];
							OFM[tii][trr][tcc] = MAX(IFM[0][tii][K_maxpool*trr+i][K_maxpool*tcc+j],OFM[tii][trr][tcc]);
						}
						else{
							if(i==0 && j==0)
								OFM[tii][trr][tcc] = IFM[1][tii][K_maxpool*trr+i][K_maxpool*tcc+j];
							OFM[tii][trr][tcc] = MAX(IFM[1][tii][K_maxpool*trr+i][K_maxpool*tcc+j],OFM[tii][trr][tcc]);
						}
					}
				}
			}
...
void maxpool_OFM_STORE_NONLINEAR(hls::stream<DMA_DATA_128B_FIX> &output_dma_O,
		FPGA_DATA_FIX OFM[Tm][Tr_maxpool][Tc_maxpool],
...
void maxpool_OFM_STORE( hls::stream<DMA_DATA_128B_FIX> &output_dma_O,
	...
	maxpool_OFM_STORE_NONLINEAR(output_dma_O,OFM,custom_Tr,custom_Tc,custom_Tm,NL_Opt);
...
void maxpool_Load_Fire(hls::stream<DMA_DATA_128B_FIX> &input_dma_W,
		hls::stream<DMA_DATA_128B_FIX> &input_dma_I,
		FPGA_DATA_FIX IFM1[2][Tn_8][K_maxpool*Tr_maxpool][K_maxpool*Tc_maxpool],
		FPGA_DATA_FIX IFM2[2][Tn_8][K_maxpool*Tr_maxpool][K_maxpool*Tc_maxpool],
		FPGA_DATA_FIX OFM[Tm][Tr_maxpool][Tc_maxpool],
		int row,
		int col,
		int N,
	...
	maxpool_LOAD_IFM(input_dma_I, IFM1, custom_Tr, custom_Tc, custom_k, custom_Tn);
	maxpool_FIRE(IFM1, OFM, row, col, custom_Tr, custom_Tc, custom_Tm, custom_Tn);
	}
...
#pragma HLS INTERFACE axis port=output_dma_O
	...
	static FPGA_DATA_FIX IFM[2][Tn_8][K_maxpool*Tr_maxpool][K_maxpool*Tc_maxpool];
#pragma HLS ARRAY_PARTITION variable=IFM complete dim=1
#pragma HLS ARRAY_PARTITION variable=IFM complete dim=2

	static FPGA_DATA_FIX IFM_DB[2][Tn_8][K_maxpool*Tr_maxpool][K_maxpool*Tc_maxpool];
#pragma HLS ARRAY_PARTITION variable=IFM_DB complete dim=1
#pragma HLS ARRAY_PARTITION variable=IFM_DB complete dim=2

	static FPGA_DATA_FIX OFM[Tm][Tr_maxpool][Tc_maxpool];
#pragma HLS RESOURCE variable=OFM core=RAM_S2P_BRAM
#pragma HLS ARRAY_PARTITION variable=OFM complete dim=1

	static FPGA_DATA_FIX OFM_DB[Tm][Tr_maxpool][Tc_maxpool];
#pragma HLS RESOURCE variable=OFM_DB core=RAM_S2P_BRAM
#pragma HLS ARRAY_PARTITION variable=OFM_DB complete dim=1


	if(num%2==0){
		maxpool_Load_Fire(input_dma_W,input_dma_I,IFM,IFM_DB,OFM_DB,row,col,N,custom_k,custom_Tr,custom_Tc,custom_Tm,custom_Tn);
		maxpool_OFM_STORE(output_dma_O,input_dma_B, OFM, custom_Tr,custom_Tc,custom_Tm,NL_Opt);
	}else{
		maxpool_Load_Fire(input_dma_W,input_dma_I,IFM,IFM_DB,OFM,row,col,N,custom_k,custom_Tr,custom_Tc,custom_Tm,custom_Tn);
		maxpool_OFM_STORE(output_dma_O,input_dma_B, OFM_DB,custom_Tr,custom_Tc,custom_Tm,NL_Opt);
	}
