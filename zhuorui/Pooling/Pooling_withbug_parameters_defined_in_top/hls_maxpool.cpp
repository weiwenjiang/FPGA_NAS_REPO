...
void maxpool_LOAD_IFM(hls::stream<DMA_DATA_128B_FIX> &input_dma_I,
		FPGA_DATA_FIX IFM[2][Tn_8][2*Tr_maxpool][2*Tc_maxpool],
		int custom_Tr,
		int custom_Tc,
		int custom_k,
		int custom_Tn
	...
	ifm1:for(int i=0;i<divided_Tn_8;i++){
		ifm2:for(int j=0;j<2*custom_Tr;j++){
			ifm3:for(int m=0;m<2*custom_Tc;m++){
...
void maxpool_FIRE(FPGA_DATA_FIX IFM[2][Tn_8][2*Tr_maxpool][2*Tc_maxpool],
...
#pragma HLS dependence variable=OFM inter false
					for(int tii=0;tii<custom_Tn;tii++){
						if(tii<Tn/2){
							if(i==0 && j==0)
								OFM[tii][trr][tcc] = IFM[0][tii][2*trr+i][2*tcc+j];
							OFM[tii][trr][tcc] = MAX(IFM[0][tii][2*trr+i][2*tcc+j],OFM[tii][trr][tcc]);
						}
						else{
							if(i==0 && j==0)
								OFM[tii][trr][tcc] = IFM[1][tii][2*trr+i][2*tcc+j];
							OFM[tii][trr][tcc] = MAX(IFM[1][tii][2*trr+i][2*tcc+j],OFM[tii][trr][tcc]);
						}
					}
				}
			}
...
void maxpool_Load_Fire(hls::stream<DMA_DATA_128B_FIX> &input_dma_W,
		...
		FPGA_DATA_FIX IFM1[2][Tn_8][2*Tr_maxpool][2*Tc_maxpool],
		FPGA_DATA_FIX IFM2[2][Tn_8][2*Tr_maxpool][2*Tc_maxpool],
		FPGA_DATA_FIX OFM[Tm][Tr_maxpool][Tc_maxpool],
		int row,
		int col,
		int N,
...
#pragma HLS INTERFACE axis port=output_dma_O
	...
	static FPGA_DATA_FIX IFM[2][Tn_8][2*Tr_maxpool][2*Tc_maxpool];
#pragma HLS ARRAY_PARTITION variable=IFM complete dim=1
#pragma HLS ARRAY_PARTITION variable=IFM complete dim=2

	static FPGA_DATA_FIX IFM_DB[2][Tn_8][2*Tr_maxpool][2*Tc_maxpool];
