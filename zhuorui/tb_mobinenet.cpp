...
int main(){
	...
	// Initialization
	extern FPGA_DATA_FIX input[CH4*(R)*(C)];//extern FPGA_DATA_FIX input[CH1*(R+2)*(C+2)];
...
*/    //
	do_conv_pad(6*CH4,CH4,R,C,R+2,C+2,1,1,input,weight_7,y_7,bias_7,input_dma_W,input_dma_I,input_dma_B,output_dma_O);
	...
	for(int m=0;m<CH5;m++){
		for(int r=0;r<real_R;r++){
			for(int c=0;c<real_C;c++){
				int res;
				int g_res;
				res = z_9[m*(real_R)*(real_C) + r*(real_C) + c];
