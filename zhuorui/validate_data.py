...
FPGA_DATA_FIX input[''', end="", file=fh)
    x = torch.randint(1,3,(1, 3, 12, 12), dtype=torch.float)
    # x = torch.zeros((1, 3, 12, 12))  # (1, 3, 12, 12)
    pad1 = nn.ZeroPad2d(1)
    # x_pad = pad1(x)
    # x_list = x_pad.flatten().tolist()

    print_list(x.flatten(), fh)
    ...
    for layer_name, layer in model.named_modules():
        if isinstance(layer, nn.Conv2d):
            ...
            #                     dtype=torch.float)
            para_w = torch.randint(-1,1,para_w.size(), dtype=torch.float)
            ...
            para_V = torch.randint(30, 33, para_V.size(), dtype=torch.float)  # 30,33
