...
void LOAD_IFM(hls::stream<DMA_DATA_128B_FIX> &input_dma_I,
		FPGA_DATA_FIX IFM[2][Tn_8][Tr+18*2+1-1][Tc+18*2+1-1],
		int custom_Tr,
		int custom_Tc,
		int custom_Tn,
		int K1
	...
	ifm1:for(int i=0;i<divided_Tn_8;i++){
		ifm2:for(int j=0;j<custom_Tr+K1-1;j++){
			ifm3:for(int m=0;m<custom_Tc+K1-1;m++){
#pragma HLS PIPELINE II=1
...
#pragma HLS dependence variable=OFM inter false
					for(int too=0;too<custom_Tm; too++){
						for(int tii=0;tii<custom_Tn;tii++){
							...
							if(too<Tm/2)
								add_res1 = WEIGHT1[0][too][tii][i][j]*IFM[0][tii][trr+dilation*i][tcc+dilation*j];
							else
								add_res1 = WEIGHT1[1][too][tii][i][j]*IFM[1][tii][trr+dilation*i][tcc+dilation*j];
...
void OFM_STORE_NONLINEAR(hls::stream<DMA_DATA_128B_FIX> &output_dma_O,
	...
	#pragma HLS PIPELINE II=1
					...
					if(i*8+1<custom_Tm)
						output_dma_O_data.data.data2=OFM[i*8+1][j][k]+BIAS[i*8+1]>=0?OFM[i*8+1][j][k]+BIAS[i*8+1]:0;
					if(i*8+2<custom_Tm)
						output_dma_O_data.data.data3=OFM[i*8+2][j][k]+BIAS[i*8+2]>=0?OFM[i*8+2][j][k]+BIAS[i*8+2]:0;
					if(i*8+3<custom_Tm)
						output_dma_O_data.data.data4=OFM[i*8+3][j][k]+BIAS[i*8+3]>=0?OFM[i*8+3][j][k]+BIAS[i*8+3]:0;
					if(i*8+4<custom_Tm)
						output_dma_O_data.data.data5=OFM[i*8+4][j][k]+BIAS[i*8+4]>=0?OFM[i*8+4][j][k]+BIAS[i*8+4]:0;
					if(i*8+5<custom_Tm)
						output_dma_O_data.data.data6=OFM[i*8+5][j][k]+BIAS[i*8+5]>=0?OFM[i*8+5][j][k]+BIAS[i*8+5]:0;
					if(i*8+6<custom_Tm)
						output_dma_O_data.data.data7=OFM[i*8+6][j][k]+BIAS[i*8+6]>=0?OFM[i*8+6][j][k]+BIAS[i*8+6]:0;
					if(i*8+7<custom_Tm)
						output_dma_O_data.data.data8=OFM[i*8+7][j][k]+BIAS[i*8+7]>=0?OFM[i*8+7][j][k]+BIAS[i*8+7]:0;
					output_dma_O.write(output_dma_O_data);
				}
			}
		}
	...
	#pragma HLS PIPELINE II=1
					...
					if(i*8+0<custom_Tm){
						FPGA_DATA_FIX d1 = OFM[i*8+0][j][k]+BIAS[i*8+0]+3>=0?OFM[i*8+0][j][k]+BIAS[i*8+0]+3:0;
						d1 = (d1<=6?d1:6)/float(6)*(OFM[i*8+0][j][k]+BIAS[i*8+0]);
						output_dma_O_data.data.data1=d1;
					}
					if(i*8+1<custom_Tm){
						FPGA_DATA_FIX d2 = OFM[i*8+1][j][k]+BIAS[i*8+1]+3>=0?OFM[i*8+1][j][k]+BIAS[i*8+1]+3:0;
						d2 = (d2<=6?d2:6)/float(6)*(OFM[i*8+1][j][k]+BIAS[i*8+1]);
						output_dma_O_data.data.data2=d2;
					}
					if(i*8+2<custom_Tm){
						FPGA_DATA_FIX d3 = OFM[i*8+2][j][k]+BIAS[i*8+2]+3>=0?OFM[i*8+2][j][k]+BIAS[i*8+2]+3:0;
						d3 = (d3<=6?d3:6)/float(6)*(OFM[i*8+2][j][k]+BIAS[i*8+2]);
						output_dma_O_data.data.data3=d3;
					}
					if(i*8+3<custom_Tm){
						FPGA_DATA_FIX d4 = OFM[i*8+3][j][k]+BIAS[i*8+3]+3>=0?OFM[i*8+3][j][k]+BIAS[i*8+3]+3:0;
						d4 = (d4<=6?d4:6)/float(6)*(OFM[i*8+3][j][k]+BIAS[i*8+3]);
						output_dma_O_data.data.data4=d4;
					}
					if(i*8+4<custom_Tm){
						FPGA_DATA_FIX d5 = OFM[i*8+4][j][k]+BIAS[i*8+4]+3>=0?OFM[i*8+4][j][k]+BIAS[i*8+4]+3:0;
						d5 = (d5<=6?d5:6)/float(6)*(OFM[i*8+4][j][k]+BIAS[i*8+4]);
						output_dma_O_data.data.data5=d5;
					}
					if(i*8+5<custom_Tm){
						FPGA_DATA_FIX d6 = OFM[i*8+5][j][k]+BIAS[i*8+5]+3>=0?OFM[i*8+5][j][k]+BIAS[i*8+5]+3:0;
						d6 = (d6<=6?d6:6)/float(6)*(OFM[i*8+5][j][k]+BIAS[i*8+5]);
						output_dma_O_data.data.data6=d6;
					}
					if(i*8+6<custom_Tm){
						FPGA_DATA_FIX d7 = OFM[i*8+6][j][k]+BIAS[i*8+6]+3>=0?OFM[i*8+6][j][k]+BIAS[i*8+6]+3:0;
						d7 = (d7<=6?d7:6)/float(6)*(OFM[i*8+6][j][k]+BIAS[i*8+6]);
						output_dma_O_data.data.data7=d7;
					}
					if(i*8+7<custom_Tm){
						FPGA_DATA_FIX d8 = OFM[i*8+7][j][k]+BIAS[i*8+7]+3>=0?OFM[i*8+7][j][k]+BIAS[i*8+7]+3:0;
						d8 = (d8<=6?d8:6)/float(6)*(OFM[i*8+7][j][k]+BIAS[i*8+7]);
...
void Load_Fire(hls::stream<DMA_DATA_128B_FIX> &input_dma_W,
		...
		FPGA_DATA_FIX IFM1[2][Tn_8][Tr+18*2+1-1][Tc+18*2+1-1],
		FPGA_DATA_FIX IFM2[2][Tn_8][Tr+18*2+1-1][Tc+18*2+1-1],
		FPGA_DATA_FIX OFM[Tm][Tr][Tc],
		int row,
		int col,
		int N,
	...
	for(int i=0;i<N+1;i+=Tn){  //tile's number
...
#pragma HLS dependence variable=WEIGHT intra false
		if(idx%2==0){   //decide the alternatively using of buffers
			...
			LOAD_IFM(input_dma_I,IFM1,custom_Tr,custom_Tc,custom_Tn,K1);
			FIRE(WEIGHT[1],IFM2,OFM, row, col, custom_k,custom_Tr,custom_Tc,custom_Tm,custom_Tn,dilation);
		}else{
			LOAD_WEIGHT_128B(input_dma_W,WEIGHT[1], custom_k,custom_Tm,custom_Tn);
			LOAD_IFM(input_dma_I,IFM2,custom_Tr,custom_Tc,custom_Tn,K1);
			FIRE(WEIGHT[0],IFM1,OFM, row, col, custom_k,custom_Tr,custom_Tc,custom_Tm,custom_Tn,dilation);
		}
		idx+=1;
	}
}
...
#pragma HLS ARRAY_PARTITION variable=WEIGHT1 complete dim=4
	...
	static FPGA_DATA_FIX IFM[2][Tn_8][Tr+18*2+1-1][Tc+18*2+1-1];
#pragma HLS ARRAY_PARTITION variable=IFM complete dim=1
#pragma HLS ARRAY_PARTITION variable=IFM complete dim=2

	static FPGA_DATA_FIX IFM_DB[2][Tn_8][Tr+18*2+1-1][Tc+18*2+1-1];
...
#pragma HLS ARRAY_PARTITION variable=BIAS_DB complete dim=1
	...
	if(num%2==0){ //decide the alternatively using of buffers
		Load_Fire(input_dma_W,input_dma_I,WEIGHT1,IFM,IFM_DB,OFM_DB,row,col,N,custom_k,custom_Tr,custom_Tc,custom_Tm,custom_Tn,K1,dilation);
		OFM_STORE(output_dma_O,input_dma_B, OFM, BIAS,custom_Tr,custom_Tc,custom_Tm,NL_Opt);
	}else{
		Load_Fire(input_dma_W,input_dma_I,WEIGHT1,IFM,IFM_DB,OFM,row,col,N,custom_k,custom_Tr,custom_Tc,custom_Tm,custom_Tn,K1,dilation);
