...
//	for(int ich=0; ich<ich_max+1;ich++){
		for(int i=0;i<divided_Tn_8_dconv;i++){
			for(int j=0;j<(custom_Tr-1)*custom_s+K_dconv;j++){
				for(int m=0;m<(custom_Tc-1)*custom_s+K_dconv;m++){
	...
	for(row=0; row<row_max;row++){
		for(col=0; col<col_max;col++){
			...
			for(mch=0; mch<mch_max+1;mch++){
				...
				stream_IFM_in_dconv(M,N,R,C,custom_s,row, col, mch, ich_max, row_max, col_max, custom_Tr, custom_Tc, input, input_dma_I);
