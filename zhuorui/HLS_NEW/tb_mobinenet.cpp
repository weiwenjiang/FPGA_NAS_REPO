...
/*	//64, s=2, n=4
	...
	/**/
	extern FPGA_DATA_FIX bn_y_5[24576];
	extern FPGA_DATA_FIX conv_y_5[24576];
	extern FPGA_DATA_FIX bn_y_7[36867];
...
*/
	...
	do_conv(CH4,6*CH3,R/4,C/4,1,1,/*z_5*/bn_y_5,weight_6,y_6,bias_6,input_dma_W,input_dma_I,input_dma_B,output_dma_O);
	...
	do_dconv(6*CH4,6*CH4,R/4+2,C/4+2,1,/*z_7*/bn_y_7,weight_8,y_8,bias_8,input_dma_W,input_dma_I,input_dma_B,output_dma_O);
...
*/
	...
	for(int m=0;m<6*CH4;m++){
		for(int r=0;r<real_R;r++){
			for(int c=0;c<real_C;c++){
				int res;
				int g_res;
				res = y_7[m*(real_R)*(real_C) + (r)*(real_C) + (c)];
				g_res = conv_y_7[m*(real_R)*(real_C) + (r)*(real_C) + (c)];
				printf("%d ",res-g_res);
			}
			printf("\n");
		}
