...
void LOAD_IFM(hls::stream<DMA_DATA_128B_FIX> &input_dma_I,
		FPGA_DATA_FIX IFM[2][Tn_8][Tr+K1-1][Tc+K1-1],
		int custom_Tr,
		int custom_Tc,
		int custom_Tn
		){
...
#pragma HLS dependence variable=OFM inter false
					for(int too=0;too<custom_Tm; too++){
						for(int tii=0;tii<custom_Tn;tii++){
							...
							if(too<Tm/2)
								add_res1 = WEIGHT1[0][too][tii][i][j]*IFM[0][tii][trr+2*i][tcc+2*j];
							else
								add_res1 = WEIGHT1[1][too][tii][i][j]*IFM[1][tii][trr+2*i][tcc+2*j];
...
void Load_Fire(hls::stream<DMA_DATA_128B_FIX> &input_dma_W,
		...
		FPGA_DATA_FIX IFM1[2][Tn_8][Tr+K1-1][Tc+K1-1],
		FPGA_DATA_FIX IFM2[2][Tn_8][Tr+K1-1][Tc+K1-1],
		FPGA_DATA_FIX OFM[Tm][Tr][Tc],
		int row,
		int col,
		int N,
