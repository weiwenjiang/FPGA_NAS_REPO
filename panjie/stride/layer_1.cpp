...
int stream_IFM_in(int M,int N,int R,int C,
	...
	for(int ich=0; ich<ich_max+1;ich++){
		for(int i=0;i<divided_Tn_8;i++){
			for(int j=0;j<(custom_Tr-1)*2+K;j++){    //change
				for(int m=0;m<(custom_Tc-1)*2+K;m++){//change
					ifm.data.data1 = ich*Tn+i*8+0<N?input[(ich*Tn+i*8+0)*R*C + (row*2*Tr + j)*C + (col*2*Tc + m)]:0;//change
					ifm.data.data2 = ich*Tn+i*8+1<N?input[(ich*Tn+i*8+1)*R*C + (row*2*Tr + j)*C + (col*2*Tc + m)]:0;
					ifm.data.data3 = ich*Tn+i*8+2<N?input[(ich*Tn+i*8+2)*R*C + (row*2*Tr + j)*C + (col*2*Tc + m)]:0;
					ifm.data.data4 = ich*Tn+i*8+3<N?input[(ich*Tn+i*8+3)*R*C + (row*2*Tr + j)*C + (col*2*Tc + m)]:0;
					ifm.data.data5 = ich*Tn+i*8+4<N?input[(ich*Tn+i*8+4)*R*C + (row*2*Tr + j)*C + (col*2*Tc + m)]:0;
					ifm.data.data6 = ich*Tn+i*8+5<N?input[(ich*Tn+i*8+5)*R*C + (row*2*Tr + j)*C + (col*2*Tc + m)]:0;
					ifm.data.data7 = ich*Tn+i*8+6<N?input[(ich*Tn+i*8+6)*R*C + (row*2*Tr + j)*C + (col*2*Tc + m)]:0;
					ifm.data.data8 = ich*Tn+i*8+7<N?input[(ich*Tn+i*8+7)*R*C + (row*2*Tr + j)*C + (col*2*Tc + m)]:0;
					if(i==divided_Tn_8-1 && j==(custom_Tr-1)*2+K-1 && m==(custom_Tc-1)*2+K-1)//change
						ifm.last = true;
					else
						ifm.last = false;
...
int do_conv(int M,int N,int R,int C,
	...
	int row_max = ceil(float(R)/(2*Tr));    //change
	int col_max = ceil(float(C)/(2*Tc));	//change
	...
	for(row=0; row<row_max;row++){
		for(col=0; col<col_max;col++){
			...
			for(mch=0; mch<mch_max+1;mch++){
				...
				if(row==row_max-1)
					custom_Tr = (R-K)/2 + 1 - row*Tr; //change
				if(col==col_max-1)
					custom_Tc = (C-K)/2 + 1 - col*Tc;//change
...
int main(){
	...
	extern FPGA_DATA_FIX weight_1[M*N*K*K];
	extern FPGA_DATA_FIX bias_1[M];
	extern FPGA_DATA_FIX output[M*((R-K)/2+1)*((C-K)/2+1)];
	FPGA_DATA_FIX my_output[M*((R-K)/2+1)*((C-K)/2+1)];
...
//	FPGA_DATA_FIX * output = (FPGA_DATA_FIX *) malloc(M*R*C*sizeof(FPGA_DATA_FIX));
	...
	// Do Conv.
	do_conv(M,N,R,C,input,weight_1,my_output,bias_1,input_dma_W,input_dma_I,input_dma_B,output_dma_O);
