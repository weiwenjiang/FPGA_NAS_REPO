...
void LOAD_WEIGHT_128B(hls::stream<DMA_DATA_128B_FIX> &input_dma_W,
	...
	weight1:for(int i=0;i<divided_Tm_8;i++){     //filter
		weight2:for(int j=0;j<custom_Tn;j++){    //kernel
			weight3:for(int m=0;m<custom_k;m++){ //kernel size
...
void LOAD_IFM(hls::stream<DMA_DATA_128B_FIX> &input_dma_I,
		FPGA_DATA_FIX IFM[2][Tn_8][2*(Tr-1)+K][2*(Tc-1)+K],
		int custom_Tr,
		int custom_Tc,
		int custom_Tn
		){
	...
	ifm1:for(int i=0;i<divided_Tn_8;i++){
		ifm2:for(int j=0;j<(custom_Tr-1)*2+K;j++){        //change
			ifm3:for(int m=0;m<(custom_Tc-1)*2+K;m++){   //change
#pragma HLS PIPELINE II=1
				ifm_input_dma=input_dma_I.read();
				if(i*8<custom_Tn){
					IFM[0][i*8][j][m]=ifm_input_dma.data.data1;//two copy
...
#pragma HLS loop_tripcount min=3 max=11 avg=3
			for(int trr=row;(trr<row+custom_Tr);trr++){     //row
#pragma HLS loop_tripcount min=1 max=Tr avg=Tr
				for(int tcc=col;(tcc<col+custom_Tc);tcc++){ //column
#pragma HLS loop_tripcount min=1 max=Tc avg=Tc
#pragma HLS PIPELINE II=1
#pragma HLS dependence variable=OFM inter false
					for(int too=0;too<custom_Tm; too++){     //filter
						for(int tii=0;tii<custom_Tn;tii++){  //kernel or channel
							FPGA_DATA_FIX add_res1;
							if(too<Tm/2)
								add_res1 = WEIGHT1[0][too][tii][i][j]*IFM[0][tii][2*trr+i][2*tcc+j];
							else
								add_res1 = WEIGHT1[1][too][tii][i][j]*IFM[1][tii][2*trr+i][2*tcc+j];
...
void Load_Fire(hls::stream<DMA_DATA_128B_FIX> &input_dma_W,
		...
		FPGA_DATA_FIX IFM1[2][Tn_8][2*(Tr-1)+K][2*(Tc-1)+K],   //change
		FPGA_DATA_FIX IFM2[2][Tn_8][2*(Tr-1)+K][2*(Tc-1)+K],	//change
		FPGA_DATA_FIX OFM[Tm][Tr][Tc],
		int row,
		int col,
		int N,
...
#pragma HLS dependence variable=WEIGHT intra false
		if(idx%2==0){   //decide the alternatively using of buffers
			...
			LOAD_IFM(input_dma_I,IFM1,custom_Tr,custom_Tc,custom_Tn);
			...
			LOAD_IFM(input_dma_I,IFM2,custom_Tr,custom_Tc,custom_Tn);
...
//	static FPGA_DATA_FIX IFM[2][Tn_8][Tr+K-1][Tc+K-1];
	static FPGA_DATA_FIX IFM[2][Tn_8][(Tr-1)*2+K][(Tc-1)*2+K]; //change
...
//	static FPGA_DATA_FIX IFM_DB[2][Tn_8][Tr+K-1][Tc+K-1];
	static FPGA_DATA_FIX IFM_DB[2][Tn_8][(Tr-1)*2+K][(Tc-1)*2+K];//change
