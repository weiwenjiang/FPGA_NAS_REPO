...
//	for(int ich=0; ich<ich_max+1;ich++){
		for(int i=0;i<divided_Tn_8_dconv;i++){
			for(int j=0;j<custom_Tr+K_dconv-1;j++){
				for(int m=0;m<custom_Tc+K_dconv-1;m++){
					ifm.data.data1 = ich*Tn_dconv+i*8+0<N?input[(ich*Tn_dconv+i*8+0)*R*C + (row*Tr_dconv + j)*C + (col*Tc_dconv + m)]:0;
					ifm.data.data2 = ich*Tn_dconv+i*8+1<N?input[(ich*Tn_dconv+i*8+1)*R*C + (row*Tr_dconv + j)*C + (col*Tc_dconv + m)]:0;
					ifm.data.data3 = ich*Tn_dconv+i*8+2<N?input[(ich*Tn_dconv+i*8+2)*R*C + (row*Tr_dconv + j)*C + (col*Tc_dconv + m)]:0;
					ifm.data.data4 = ich*Tn_dconv+i*8+3<N?input[(ich*Tn_dconv+i*8+3)*R*C + (row*Tr_dconv + j)*C + (col*Tc_dconv + m)]:0;
					ifm.data.data5 = ich*Tn_dconv+i*8+4<N?input[(ich*Tn_dconv+i*8+4)*R*C + (row*Tr_dconv + j)*C + (col*Tc_dconv + m)]:0;
					ifm.data.data6 = ich*Tn_dconv+i*8+5<N?input[(ich*Tn_dconv+i*8+5)*R*C + (row*Tr_dconv + j)*C + (col*Tc_dconv + m)]:0;
					ifm.data.data7 = ich*Tn_dconv+i*8+6<N?input[(ich*Tn_dconv+i*8+6)*R*C + (row*Tr_dconv + j)*C + (col*Tc_dconv + m)]:0;
					ifm.data.data8 = ich*Tn_dconv+i*8+7<N?input[(ich*Tn_dconv+i*8+7)*R*C + (row*Tr_dconv + j)*C + (col*Tc_dconv + m)]:0;
					if(i==divided_Tn_8_dconv-1 && j==custom_Tr-1+K_dconv-1 && m==custom_Tc-1+K_dconv-1)
						ifm.last = true;
					else
						ifm.last = false;
...
int stream_OFM_out_dconv(int M,int N,int R,int C,
	...
	for(int i=0;i<divided_Tn_8_dconv;i++){
		for(int j=0;j<custom_Tr;j++){
			for(int m=0;m<custom_Tc;m++){
				...
				if(mch!=-1){
					if(mch*Tm + i*8+0<M)
						output[(mch*Tn_dconv + i*8+0)*(R-K_dconv+1)*(C-K_dconv+1) + (row*Tr_dconv + j)*(C-K_dconv+1) + (col*Tc_dconv + m)] = output_dma_O_data.data.data1;
					if(mch*Tm + i*8+1<M)
						output[(mch*Tn_dconv + i*8+1)*(R-K_dconv+1)*(C-K_dconv+1) + (row*Tr_dconv + j)*(C-K_dconv+1) + (col*Tc_dconv + m)] = output_dma_O_data.data.data2;
					if(mch*Tm + i*8+2<M)
						output[(mch*Tn_dconv + i*8+2)*(R-K_dconv+1)*(C-K_dconv+1) + (row*Tr_dconv + j)*(C-K_dconv+1) + (col*Tc_dconv + m)] = output_dma_O_data.data.data3;
					if(mch*Tm + i*8+3<M)
						output[(mch*Tn_dconv + i*8+3)*(R-K_dconv+1)*(C-K_dconv+1) + (row*Tr_dconv + j)*(C-K_dconv+1) + (col*Tc_dconv + m)] = output_dma_O_data.data.data4;
					if(mch*Tm + i*8+4<M)
						output[(mch*Tn_dconv + i*8+4)*(R-K_dconv+1)*(C-K_dconv+1) + (row*Tr_dconv + j)*(C-K_dconv+1) + (col*Tc_dconv + m)] = output_dma_O_data.data.data5;
					if(mch*Tm + i*8+5<M)
						output[(mch*Tn_dconv + i*8+5)*(R-K_dconv+1)*(C-K_dconv+1) + (row*Tr_dconv + j)*(C-K_dconv+1) + (col*Tc_dconv + m)] = output_dma_O_data.data.data6;
					if(mch*Tm + i*8+6<M)
						output[(mch*Tn_dconv + i*8+6)*(R-K_dconv+1)*(C-K_dconv+1) + (row*Tr_dconv + j)*(C-K_dconv+1) + (col*Tc_dconv + m)] = output_dma_O_data.data.data7;
					if(mch*Tm + i*8+7<M)
						output[(mch*Tn_dconv + i*8+7)*(R-K_dconv+1)*(C-K_dconv+1) + (row*Tr_dconv + j)*(C-K_dconv+1) + (col*Tc_dconv + m)] = output_dma_O_data.data.data8;
				}
			}
		}
	}
...
//				printf("%d %d %d: %d,%d\n",mch, row, col, custom_Tr,custom_Tc);
				...
				db_flag+=1;
				stream_OFM_out_dconv(M,N,R,C,row,col,mch-1,row_max,col_max, custom_Tr, custom_Tc, output,output_dma_O);
			}
		}
	}
