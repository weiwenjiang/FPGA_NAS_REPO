		...
		for(int i=0;i<divided_Tn_8;i++){
			for(int j=0;j<custom_Tr+K-1;j++){
				for(int m=0;m<custom_Tc+K-1;m++){
					ifm.data.data1 = ich*Tn+i*8+0<N?input[(ich*Tn+i*8+0)*R*C + (row*Tr + j)*C + (col*Tc + m)]:FPGA_DATA_FIX(0);
					ifm.data.data2 = ich*Tn+i*8+1<N?input[(ich*Tn+i*8+1)*R*C + (row*Tr + j)*C + (col*Tc + m)]:FPGA_DATA_FIX(0);
					ifm.data.data3 = ich*Tn+i*8+2<N?input[(ich*Tn+i*8+2)*R*C + (row*Tr + j)*C + (col*Tc + m)]:FPGA_DATA_FIX(0);
					ifm.data.data4 = ich*Tn+i*8+3<N?input[(ich*Tn+i*8+3)*R*C + (row*Tr + j)*C + (col*Tc + m)]:FPGA_DATA_FIX(0);
					ifm.data.data5 = ich*Tn+i*8+4<N?input[(ich*Tn+i*8+4)*R*C + (row*Tr + j)*C + (col*Tc + m)]:FPGA_DATA_FIX(0);
					ifm.data.data6 = ich*Tn+i*8+5<N?input[(ich*Tn+i*8+5)*R*C + (row*Tr + j)*C + (col*Tc + m)]:FPGA_DATA_FIX(0);
					ifm.data.data7 = ich*Tn+i*8+6<N?input[(ich*Tn+i*8+6)*R*C + (row*Tr + j)*C + (col*Tc + m)]:FPGA_DATA_FIX(0);
					ifm.data.data8 = ich*Tn+i*8+7<N?input[(ich*Tn+i*8+7)*R*C + (row*Tr + j)*C + (col*Tc + m)]:FPGA_DATA_FIX(0);
			...
			for(int i=0;i<Tn;i++){
				for(int j=0;j<custom_k;j++){
					for(int m=0;m<custom_k;m++){
						wei.data.data1 = mch*Tm + o*8+0 < M && ich*Tn+i < N ?weight[(mch*Tm + o*8+0)*N*K*K + (ich*Tn+i)*K*K + (j)*K + (m)]:FPGA_DATA_FIX(0);
						wei.data.data2 = mch*Tm + o*8+1 < M && ich*Tn+i < N ?weight[(mch*Tm + o*8+1)*N*K*K + (ich*Tn+i)*K*K + (j)*K + (m)]:FPGA_DATA_FIX(0);
						wei.data.data3 = mch*Tm + o*8+2 < M && ich*Tn+i < N ?weight[(mch*Tm + o*8+2)*N*K*K + (ich*Tn+i)*K*K + (j)*K + (m)]:FPGA_DATA_FIX(0);
						wei.data.data4 = mch*Tm + o*8+3 < M && ich*Tn+i < N ?weight[(mch*Tm + o*8+3)*N*K*K + (ich*Tn+i)*K*K + (j)*K + (m)]:FPGA_DATA_FIX(0);
						wei.data.data5 = mch*Tm + o*8+4 < M && ich*Tn+i < N ?weight[(mch*Tm + o*8+4)*N*K*K + (ich*Tn+i)*K*K + (j)*K + (m)]:FPGA_DATA_FIX(0);
						wei.data.data6 = mch*Tm + o*8+5 < M && ich*Tn+i < N ?weight[(mch*Tm + o*8+5)*N*K*K + (ich*Tn+i)*K*K + (j)*K + (m)]:FPGA_DATA_FIX(0);
						wei.data.data7 = mch*Tm + o*8+6 < M && ich*Tn+i < N ?weight[(mch*Tm + o*8+6)*N*K*K + (ich*Tn+i)*K*K + (j)*K + (m)]:FPGA_DATA_FIX(0);
						wei.data.data8 = mch*Tm + o*8+7 < M && ich*Tn+i < N ?weight[(mch*Tm + o*8+7)*N*K*K + (ich*Tn+i)*K*K + (j)*K + (m)]:FPGA_DATA_FIX(0);
...
int stream_BIAs_in(int M,int N,int R,int C,
	...
	for(int i=0;i<divided_Tm_8;i++){
		bia.data.data1 = mch*Tm + i*8+0 < M ? bias[mch*Tm + i*8+0] : FPGA_DATA_FIX(0);
		bia.data.data2 = mch*Tm + i*8+1 < M ? bias[mch*Tm + i*8+1] : FPGA_DATA_FIX(0);
		bia.data.data3 = mch*Tm + i*8+2 < M ? bias[mch*Tm + i*8+2] : FPGA_DATA_FIX(0);
		bia.data.data4 = mch*Tm + i*8+3 < M ? bias[mch*Tm + i*8+3] : FPGA_DATA_FIX(0);
		bia.data.data5 = mch*Tm + i*8+4 < M ? bias[mch*Tm + i*8+4] : FPGA_DATA_FIX(0);
		bia.data.data6 = mch*Tm + i*8+5 < M ? bias[mch*Tm + i*8+5] : FPGA_DATA_FIX(0);
		bia.data.data7 = mch*Tm + i*8+6 < M ? bias[mch*Tm + i*8+6] : FPGA_DATA_FIX(0);
		bia.data.data8 = mch*Tm + i*8+7 < M ? bias[mch*Tm + i*8+7] : FPGA_DATA_FIX(0);
		if(i==divided_Tm_8-1)
			bia.last = true;
		else
			bia.last = false;
...
int main(){
	...
	extern FPGA_DATA_FIX output[CH3*(R-K*2+2)*(C-K*2+2)];
	FPGA_DATA_FIX L1_output[CH2*(R-K+1)*(C-K+1)];
	FPGA_DATA_FIX L2_output[CH3*(R-2*K+2)*(C-2*K+2)];
...
//	FPGA_DATA_FIX * output = (FPGA_DATA_FIX *) malloc(M*R*C*sizeof(FPGA_DATA_FIX));
	...
	do_conv(CH3,CH2,R-K+1,C-K+1,L1_output,weight_2,L2_output,bias_2,input_dma_W,input_dma_I,input_dma_B,output_dma_O);


	for(int m=0;m<CH3;m++){
			for(int r=0;r<R-2*K+2;r++){
				for(int c=0;c<C-2*K+2;c++){
					//printf("%d ", L2_output[m*(R-2*K+2)*(C-2*K+2) + r*(C-2*K+2) + c]);
					std::cout << (L2_output[m*(R-2*K+2)*(C-2*K+2) + r*(C-2*K+2) + c]) <<" ";
				}
				printf("\n");
			}
			printf("========%d==========\n",m);
		...
		for(int m=0;m<CH3;m++){
			for(int r=0;r<R-2*K+2;r++){
				for(int c=0;c<C-2*K+2;c++){
					//printf("%d ", L2_output[m*(R-2*K+2)*(C-2*K+2) + r*(C-2*K+2) + c]-output[m*(R-2*K+2)*(C-2*K+2) + r*(C-2*K+2) + c]);
					std::cout << (L2_output[m*(R-2*K+2)*(C-2*K+2) + r*(C-2*K+2) + c]-output[m*(R-2*K+2)*(C-2*K+2) + r*(C-2*K+2) + c]) <<" ";
