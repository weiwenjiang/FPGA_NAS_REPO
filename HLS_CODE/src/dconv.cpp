...
void dconv_LOAD_WEIGHT_128B(hls::stream<DMA_DATA_128B_FIX> &input_dma_W,
						FPGA_DATA_FIX WEIGHT1[2][Tn][K][K],
						int custom_k){


	DMA_DATA_128B_FIX weight_input_dma;
	weight1:for(int i=0;i<divided_Tn_8;i++){
...
void dconv_FIRE(  FPGA_DATA_FIX WEIGHT1[2][Tn][K][K],
			FPGA_DATA_FIX IFM[2][Tn][Tr][Tc],
...
void dconv_OFM_STORE_NONLINEAR(hls::stream<DMA_DATA_128B_FIX> &output_dma_O,
	...
	if(NL_Opt==0){
		...
		dconv_n_ofm1:for(int i=0;i<divided_Tn_8;i++){
			dconv_n_ofm2:for(int j=0;j<custom_Tr;j++){
				dconv_n_ofm3:for(int k=0;k<custom_Tc;k++){
	#pragma HLS PIPELINE II=1
					output_dma_O_data.last=0;
					if (i==divided_Tn_8-1 && j==custom_Tr-1 && k==custom_Tc-1){
		...
		// ReLU
		dconv_r_ofm1:for(int i=0;i<divided_Tn_8;i++){
			dconv_r_ofm2:for(int j=0;j<custom_Tr;j++){
				dconv_r_ofm3:for(int k=0;k<custom_Tc;k++){
	#pragma HLS PIPELINE II=1
					output_dma_O_data.last=0;
					if (i==divided_Tn_8-1 && j==custom_Tr-1 && k==custom_Tc-1){
						output_dma_O_data.last=1;
		...
		// h-switch
		dconv_hs_ofm1:for(int i=0;i<divided_Tn_8;i++){
			dconv_hs_ofm2:for(int j=0;j<custom_Tr;j++){
				dconv_hs_ofm3:for(int k=0;k<custom_Tc;k++){
	#pragma HLS PIPELINE II=1
					output_dma_O_data.last=0;
					if (i==divided_Tn_8-1 && j==custom_Tr-1 && k==custom_Tc-1){
						output_dma_O_data.last=1;
...
//	ofm1:for(int i=0;i<divided_Tn_8;i++){
//		ofm2:for(int j=0;j<custom_Tr;j++){
//			ofm3:for(int k=0;k<custom_Tc;k++){
//#pragma HLS PIPELINE II=1
//				output_dma_O_data.last=0;
//				if (i==divided_Tn_8-1 && j==custom_Tr-1 && k==custom_Tc-1){
//					output_dma_O_data.last=1;
...
void dconv_Load_Fire(hls::stream<DMA_DATA_128B_FIX> &input_dma_W,
		...
		FPGA_DATA_FIX WEIGHT[2][2][Tn][K][K],
		FPGA_DATA_FIX IFM1[2][Tn][Tr][Tc],
		FPGA_DATA_FIX IFM2[2][Tn][Tr][Tc],
		FPGA_DATA_FIX OFM[Tn][Tr][Tc],
		int row,
		int col,
		int N,
