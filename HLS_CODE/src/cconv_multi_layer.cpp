	...
	for(int i=0;i<divided_Tm_8;i++){
		for(int j=0;j<custom_Tr;j++){
			for(int m=0;m<custom_Tc;m++){
				...
				if(mch!=-1){
					output[(mch*Tm + i*8+0)*(R-K+1)*(C-K+1) + (row*Tr + j)*(C-K+1) + (col*Tc + m)] = output_dma_O_data.data.data1;
					output[(mch*Tm + i*8+1)*(R-K+1)*(C-K+1) + (row*Tr + j)*(C-K+1) + (col*Tc + m)] = output_dma_O_data.data.data2;
					output[(mch*Tm + i*8+2)*(R-K+1)*(C-K+1) + (row*Tr + j)*(C-K+1) + (col*Tc + m)] = output_dma_O_data.data.data3;
					output[(mch*Tm + i*8+3)*(R-K+1)*(C-K+1) + (row*Tr + j)*(C-K+1) + (col*Tc + m)] = output_dma_O_data.data.data4;
					output[(mch*Tm + i*8+4)*(R-K+1)*(C-K+1) + (row*Tr + j)*(C-K+1) + (col*Tc + m)] = output_dma_O_data.data.data5;
					output[(mch*Tm + i*8+5)*(R-K+1)*(C-K+1) + (row*Tr + j)*(C-K+1) + (col*Tc + m)] = output_dma_O_data.data.data6;
					output[(mch*Tm + i*8+6)*(R-K+1)*(C-K+1) + (row*Tr + j)*(C-K+1) + (col*Tc + m)] = output_dma_O_data.data.data7;
					output[(mch*Tm + i*8+7)*(R-K+1)*(C-K+1) + (row*Tr + j)*(C-K+1) + (col*Tc + m)] = output_dma_O_data.data.data8;
				}
			}
		}
	}
...
int main(){
	...
	extern FPGA_DATA_FIX output[CH3*(R-2*K+2)*(C-2*K+2)];
